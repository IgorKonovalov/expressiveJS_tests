...
function urlToPath(url) { // принимает имя пути, декодирует от странных символов, и вставляет в начале точку чтобы получить путь к файлу в файловой системе сервера
  let path = require("url").parse(url).pathname;
  return "." + decodeURIComponent(path);
}

methods.GET = function(path, respond) {
  fs.stat(path, function(error, stats) { // объект fs.stat содержит информацию по запрашиваемому файлу
    if (error && error.code == "ENOENT") // возвращает ENOENT если файла нет
